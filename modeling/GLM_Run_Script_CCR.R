#*****************************************************************                                                           *
#* TITLE:   Run GLM-AED for CCR
#* AUTHORS:  C.C. Carey  
#* Edited by DWH  for CCR - 21 nov 2022                  
...
metabolism <- get_var(nc_file, "OXY_oxy", reference="surface", z_out=1) %>% 
  mutate(DateTime = as.POSIXct(strptime(DateTime, "%Y-%m-%d", tz="EST"))) 
plot(metabolism$DateTime, metabolism$OXY_oxy_1*32/1000, xlim=as.POSIXct(c("2015-08-01", "2015-08-20")), ylim=c(8,10))
...
for(i in 1:length(depths)){
  ...
  observed <-subset(obs, obs$Depth==depths[i])
  if(nrow(modeled)>1){
...
for(i in 1:nrow(vars)){
  ...
  tmp<- get_var(nc_file, var, reference="surface", z_out=depths) %>%
    pivot_longer(cols=starts_with(paste0(var,"_")), names_to="Depth", names_prefix=paste0(var,"_"), values_to = "value") %>%
    mutate(DateTime = as.POSIXct(strptime(DateTime, "%Y-%m-%d", tz="EST"))) %>%
    mutate(Depth=as.numeric(Depth)) %>%
    na.omit() %>% 
...
mod %>% 
  ...
  ggplot(aes(x = DateTime, y = value, color = limit)) + 
  geom_line() +
  facet_wrap(~phyto) +
  theme_bw()
...
var <- "CAR_pH"
obs<-read.csv('field_data/field_pH.csv', header=TRUE) %>% #read in observed pH data
  dplyr::mutate(DateTime = as.POSIXct(strptime(DateTime, "%Y-%m-%d", tz="EST"))) %>%
...
#metabolism
gpp <- get_var(file=nc_file,var_name = 'PHY_gpp',z_out=1,reference = 'surface') 
plot(gpp$DateTime, gpp$PHY_gpp_1, type="l")

ncp <- get_var(file=nc_file,var_name = 'PHY_ncp',z_out=1,reference = 'surface') 
plot(ncp$DateTime, ncp$PHY_ncp_1, type="l") #mmol C/m3/d

annual_ncp <- ncp %>% 
  mutate(DateTime = as.POSIXct(strptime(DateTime, "%Y-%m-%d", tz="EST")),
         Year = lubridate::year(DateTime)) %>% 
  group_by(Year) %>% 
  summarize(sum_ncp_mmolCm3yr = sum(PHY_ncp_1)) %>% 
  mutate(mgO2Lyr_ncp = sum_ncp_mmolCm3yr*12/1000)
...
obs<-read.csv('field_data/CleanedObsChla.csv', header=TRUE) %>% #read in observed chemistry data
  ...
  dplyr::select(DateTime, Depth, var) %>%
  na.omit() %>% 
  #dplyr::filter(lubridate::year(DateTime) %in% c("2017","2018")) %>% 
...
mod2 <- mod %>% pivot_wider(names_from = limit, values_from = value) %>% 
  mutate(R_growth = ifelse(phyto == "cyano", 1.0, 3.0),
         R_growth = ifelse(phyto == "green", 2.0, R_growth)) %>% 
  rowwise() %>% 
  mutate(nutrients = min(c(fI,fPho,fSil,fNit)),
         combined = fT * nutrients,
         prod = combined * R_growth) %>% 
  select(-fSal) %>% 
  pivot_longer(-c("DateTime","Depth", "phyto"), names_to = "limit", values_to = "value")
